...
Theorem transf_c_program_preservation:
  ...
  program_behaves (Asm.semantics Asm.instr_size_1 tp) beh ->
...
Theorem transf_c_program_is_refinement:
  ...
  (forall beh, program_behaves (Asm.semantics Asm.instr_size_1 tp) beh -> program_behaves (Csem.semantics (fn_stack_requirements tp)p) beh).
Proof.
...
Theorem transf_cstrategy_program_preservation:
  ...
  (forall beh, program_behaves (Cstrategy.semantics (fn_stack_requirements tp) p) beh ->
     exists beh', program_behaves (Asm.semantics Asm.instr_size_1 tp) beh' /\ behavior_improves beh beh')
/\(forall beh, program_behaves (Asm.semantics Asm.instr_size_1 tp) beh ->
...
/\(forall beh, not_wrong beh ->
     program_behaves (Cstrategy.semantics (fn_stack_requirements tp)p) beh -> program_behaves (Asm.semantics Asm.instr_size_1 tp) beh)
/\(forall beh,
...
Theorem bigstep_cstrategy_preservation:
  ...
  (forall t r,
     ...
     program_behaves (Asm.semantics Asm.instr_size_1 tp) (Terminates t r))
/\(forall T,
     Cstrategy.bigstep_program_diverges (fn_stack_requirements tp) p T ->
       program_behaves (Asm.semantics Asm.instr_size_1 tp) (Reacts T)
    \/ exists t, program_behaves (Asm.semantics Asm.instr_size_1 tp) (Diverges t) /\ traceinf_prefix t T).
Proof.
  intuition.
...
Definition ssasm_program_satisfies_spec (p: Asm.program) (spec: specification): Prop :=
  forall beh,  program_behaves (Asm.semantics Asm.instr_size_1 p) beh -> spec beh.
...
Theorem transf_c_program_preserves_spec:
  ...
  ssasm_program_satisfies_spec tp spec.
Proof.
...
Definition ssasm_program_has_initial_trace (p: Asm.program) (t: trace): Prop :=
  forall beh, program_behaves (Asm.semantics Asm.instr_size_1 p) beh -> behavior_prefix t beh.

Theorem transf_c_program_preserves_initial_trace:
  forall p tp t,
  transf_c_program p = OK tp ->
  c_program_has_initial_trace p t ->
  ssasm_program_has_initial_trace tp t.
Proof.
...
Theorem separate_transf_c_program_preservation:
  forall beh,
  program_behaves (Asm.semantics Asm.instr_size_1 asm_program) beh ->
...
Theorem separate_transf_c_program_is_refinement:
  ...
  (forall beh, program_behaves (Asm.semantics Asm.instr_size_1 asm_program) beh -> program_behaves (Csem.semantics (fn_stack_requirements asm_program)c_program) beh).
Proof.
...
Theorem separate_transf_c_program_preserves_spec:
  ...
  ssasm_program_satisfies_spec asm_program spec.
Proof.
...
Theorem separate_transf_c_program_preserves_initial_trace:
  ...
  ssasm_program_has_initial_trace asm_program t.
Proof.
