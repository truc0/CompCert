...
Proof.
  intros. destruct H. destruct H. destruct mext_inj. constructor; intros.
- replace ofs with (ofs + 0) by lia. eapply mi_perm; eauto. auto.
- eauto.
- exploit mi_memval; eauto. unfold inject_id; eauto.
  rewrite Z.add_0_r. auto.
- unfold Mem.stackeq in H0.
  destruct (Mem.support m1).
  destruct (Mem.support m2).
  simpl in *. congruence.
Qed.
...
Proof.
  intros. destruct H0. split. constructor; auto.
- congruence.
- constructor; intros.
...
Proof.
  ...
  destruct (Mem.stack(Mem.support m1)) eqn:S1. discriminate.
  destruct (Mem.stack(Mem.support m2)) eqn:S2. discriminate.
  inv H0. inv POP.
...
- (* tailcall *)
  ...
  intros (tm' & C & D).
  exploit magree_pop_stage; eauto.
  intros (tm''& E & F).
  econstructor; split.
