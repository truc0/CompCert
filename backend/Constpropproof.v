...
+ (* operator is strength-reduced *)
  ...
  { eapply eval_operation_lessdef; eauto. eapply regs_lessdef_regs; eauto. apply MEM. }
...
+ (* strength-reduced *)
  ...
  intros (a'' & U & V).
  assert (W: eval_addressing tge (Vptr (fresh_block sps) Ptrofs.zero) addr' rs'##args' = Some a'').
...
- (* Icall *)
  ...
  eapply sig_function_translated. auto.
...
- (* Icond, preserved *)
  ...
  intros EV1 TCODE.
  left; exists O; exists (State s' (transf_function (romem_for cu) f) (Vptr (fresh_block sps) Ptrofs.zero) (if b then ifso else ifnot) rs' m'); split.
...
- (* Ijumptable *)
  ...
  left; exists O; exists (State s' (transf_function (romem_for cu) f) (Vptr (fresh_block sps) Ptrofs.zero) pc' rs' m'); split.
