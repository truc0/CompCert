...
Definition find_offset (id:ident) :=
  match Genv.find_funct_ptr ge (Global id) with
    |Some (Internal f) => Some (fe_stack_data (make_env (function_bounds f)))
    |_ => None
  end.
...
- set (ty := mreg_type r) in *.
  ...
  split. unfold store_stack in STORE. erewrite Mem.support_storev; eauto.
  auto.
Qed.
...
Lemma function_prologue_correct:
  forall j ls ls0 ls1 rs rs1 m0 m0' m1 m2 m3 fr sp parent ra cs fb k P id path,
  agree_regs j ls rs ->
  agree_callee_save ls ls0 ->
  ...
  j = struct_meminj (Mem.support m0) ->
(
  exists j', exists rs',exists m1', exists m2', exists sp', exists m3', exists m4', exists m5', exists m6',
     Mem.alloc_frame m0' id = (m1',path)
  ...
  /\ j' = struct_meminj (Mem.support m3)).
Proof.
  intros until path; intros AGREGS AGCS AGARGS WTREGS LS1 RS1 FIND ALLOCF ALLOC RECORD TYPAR TYRA SEP STK ASTK SINJ.
...
Local Opaque b fe.
  ...
  assert (path = path').
  exploit Mem.alloc_frame_parallel_stackeq. apply ALLOCF. apply ALLOCF'.
  eauto. intros [A B]. auto. subst path'.
  ...
  tauto.
  tauto.
  clear SEP. intros (j' & SEP & INCR & SAME1 & SAME2).
  ...
  rewrite (Mem.astack_alloc _ _ _ _ _ ALLOC).
  rewrite (Mem.astack_alloc _ _ _ _ _ ALLOC').
  rewrite <- (Mem.astack_alloc_frame _ _ _ _ ALLOCF).
  rewrite <- (Mem.astack_alloc_frame _ _ _ _ ALLOCF').
  rewrite ASTK. lia.
  ...
  assert (SEPCONJ:
    m3' |= mconj (range sp' 0 (fe_stack_data fe) ** range sp' (fe_stack_data fe + bound_stack_data b) (fe_size fe))
                 (range sp' 0 (fe_stack_data fe) ** range sp' (fe_stack_data fe + bound_stack_data b) (fe_size fe))
           ** minjection j' m3 ** globalenv_inject ge j' ** P).
  ...
  clear SEP; intros (rs2 & m6' & SAVE_CS & SEP & PERMS & SUP' & AGREGS').
  rewrite sep_swap5 in SEP.
  ...
  rewrite sep_swap in SEP.
  (* Now we frame this *)
  assert (SEPFINAL: m6' |= frame_contents j' sp' ls1 ls0 parent ra ** minjection j' m3 ** globalenv_inject ge j' ** P).
  { eapply frame_mconj. eexact SEPCONJ.
    rewrite chunk_of_Tptr in SEP.
    unfold frame_contents_1; rewrite ! sep_assoc. exact SEP.
    assert (forall ofs k p, Mem.perm m3' sp' ofs k p -> Mem.perm m6' sp' ofs k p).
    { intros. apply PERMS.
  ...
  clear SEP SEPCONJ.
  assert (STK'': Mem.stackeq m3 m6').
   exploit Mem.alloc_frame_parallel_stackeq. 3: eauto.
   eauto. eauto. intros [X Y].
   exploit Mem.alloc_parallel_stackeq. 3: eauto. eauto. eauto.
   intros [X' Y'].
   apply Mem.support_storev in STORE_RETADDR.
   apply Mem.support_storev in STORE_PARENT.
   apply Mem.sid_record_frame in RECORD as RS.
   apply Mem.sid_record_frame in RECORD' as RS'.
   apply Mem.stack_record_frame in RECORD.
   apply Mem.stack_record_frame in RECORD'.
   rewrite Mem.stack_push_stage in RECORD'.
   rewrite Mem.stack_push_stage in RECORD.
   inv X'. split. congruence. simpl in *. congruence.
 assert (ASTK'': Mem.astack (Mem.support m3) = Mem.astack (Mem.support m6')).
   exploit Mem.alloc_frame_parallel_astackeq; eauto. intro.
   exploit Mem.alloc_parallel_astackeq. 3: eauto. eauto. eauto. intro.
   apply Mem.support_storev in STORE_RETADDR.
   apply Mem.support_storev in STORE_PARENT.
   apply Mem.astack_record_frame in RECORD.
   apply Mem.astack_record_frame in RECORD'.
   rewrite Mem.astack_push_stage in RECORD'.
   rewrite Mem.astack_push_stage in RECORD.
   destruct RECORD as (x&y&X&Y). destruct RECORD' as (xx&yy&XX&YY).
   congruence.
  assert (SINJ' : j' = struct_meminj (Mem.support m3)).
  apply Axioms.extensionality. intro b0.
  destruct (eq_block b0 sp). subst.
  rewrite SAME1. unfold struct_meminj.
  apply Mem.valid_new_block in ALLOC as H0. destr; (try congruence).
  unfold unchecked_meminj.
  exploit Mem.alloc_frame_alloc; eauto. intro.
  exploit Mem.alloc_frame_alloc. apply ALLOCF. eauto. intro.
  subst.
  assert (find_offset id = Some (fe_stack_data fe)).
  unfold find_offset. rewrite FIND. auto.
  rewrite H.
  apply Mem.sid_alloc_frame in ALLOCF. apply Mem.sid_alloc_frame in ALLOCF'.
  rewrite <- ALLOCF'. rewrite <- ALLOCF. inv STK. rewrite H2. auto.
...
(* Conclusions *)
  exists j', rs2,m1',m2', sp', m3', m4', m5', m6'.
  split. auto.
  split. auto.
  split. auto.
...
Fixpoint stack_contents (j: meminj) (cs: list Linear.stackframe) (cs': list Mach.stackframe)(sid:nat) : massert :=
  match cs, cs' with
  | nil, nil => pure True
  | Linear.Stackframe f _ ls c :: cs, Mach.Stackframe fb (Vptr sp' _) ra c' :: cs' =>
      frame_contents f j sp' ls (parent_locset cs) (parent_sp sid cs') (parent_ra cs')
      ** stack_contents j cs cs' sid
  | _, _ => pure False
  end.
...
Lemma stack_contents_change_meminj:
  ...
  forall cs cs' P sid ,
  m |= stack_contents j cs cs' sid ** P ->
  m |= stack_contents j' cs cs' sid ** P.
Proof.
Local Opaque sepconj.
...
Lemma match_stacks_type_sp:
  forall j cs cs' sg sid,
  match_stacks j cs cs' sg ->
  Val.has_type (parent_sp sid cs') Tptr.
Proof.
...
Variable m': mem.
Hypothesis SEP: m' |= stack_contents j cs cs' (Mem.sid (Mem.support m')).

Lemma transl_external_argument:
  forall l,
  In l (regs_of_rpairs (loc_arguments sg)) ->
  exists v, extcall_arg rs m' (parent_sp (Mem.sid(Mem.support m'))cs') l v /\ Val.inject j (ls l) v.
Proof.
  intros.
...
Lemma transl_external_argument_2:
  ...
  exists v, extcall_arg_pair rs m' (parent_sp (Mem.sid (Mem.support m'))cs') p v /\ Val.inject j (Locmap.getpair p ls) v.
Proof.
...
Lemma transl_external_arguments_rec:
  ...
  exists vl,
      list_forall2 (extcall_arg_pair rs m' (parent_sp (Mem.sid(Mem.support m'))cs')) locs vl
...
Lemma transl_external_arguments:
  exists vl,
      extcall_arguments rs m' (parent_sp (Mem.sid(Mem.support m'))cs') sg vl
...
Inductive match_states: Linear.state -> Mach.state -> Prop :=
  | match_states_intro:
      forall cs f sp c ls m cs' fb sp' rs m' j tf
        ...
        (SINJ : j = struct_meminj (Mem.support m))
        (SEP: m' |= frame_contents f j sp' ls (parent_locset cs) (parent_sp (Mem.sid(Mem.support m')) cs') (parent_ra cs')
                 ** stack_contents j cs cs' (Mem.sid(Mem.support m'))
                 ** minjection j m
  ...
  | match_states_call:
      forall cs f ls m cs' rs m' j tf id
        ...
        (ASTK: Mem.astack (Mem.support m) = Mem.astack (Mem.support m'))
        (SINJ : j = struct_meminj (Mem.support m))
        (SEP: m' |= stack_contents j cs cs' (Mem.sid(Mem.support m'))
                 ** minjection j m
                 ** globalenv_inject ge j),
      match_states (Linear.Callstate cs f ls m id)
                   (Mach.Callstate cs' (Global id) rs m' id)
  | match_states_return:
...
Lemma find_function_inv : forall ros rs id f,
    Linear.ros_is_ident ros rs id ->
    find_function ge ros rs = Some f ->
    Genv.find_funct_ptr ge (Global id) = Some f.
Proof.
  intros. destruct ros; simpl in *.
  rewrite H in H0. simpl in H0. destr_in H0. destr_in H0.
...
- (* Lsetstack *)
  ...
  assert (A: exists m'',
           ...
           /\ m'' |= frame_contents f (struct_meminj (Mem.support m)) sp' (Locmap.set (S sl ofs ty) (rs (R src))
                                               (LTL.undef_regs (destroyed_by_setstack ty) rs))
                                            (parent_locset s) (parent_sp (Mem.sid(Mem.support m')) cs') (parent_ra cs')
                  ** stack_contents (struct_meminj (Mem.support m)) s cs' (Mem.sid(Mem.support m')) ** minjection (struct_meminj (Mem.support m)) m ** globalenv_inject ge (struct_meminj (Mem.support m))).
  ...
  eauto. eauto with coqlib. unfold Mem.stackeq in *.
...
- (* Lop *)
  assert (exists v',
       ...
       /\ Val.inject (struct_meminj (Mem.support m)) v v').
...
- (* Lload *)
  assert (exists a',
       ...
       /\ Val.inject (struct_meminj (Mem.support m)) a a').
...
- (* Lstore *)
  ...
  rewrite <- (Mem.support_storev _ _ _ _ _ C). auto.
  rewrite <- (Mem.support_storev _ _ _ _ _ H0). auto.
...
- (* Lcall *)
  ...
  econstructor; eauto. econstructor; eauto with coqlib.
  apply Val.Vptr_has_type. intros; red.
...
- (* Ltailcall *)
  rewrite (sep_swap (stack_contents (struct_meminj (Mem.support m)) s cs' (Mem.sid(Mem.support m'0)))) in SEP.
  exploit function_epilogue_correct; eauto.
  clear SEP. intros (rs1 & m1' & P & Q & R & S & T & U & SEP).
  apply Mem.support_free in R as SF. apply Mem.support_free in H3 as SF'.
  exploit return_frame_parallel_rule; eauto.
  apply Mem.return_frame_active in H4. inv STK. congruence.
  clear SEP. intros (m2' & RET & SEP).
  apply Mem.astack_return_frame in H4 as ASRET.
  apply Mem.astack_return_frame in RET as ASRET'.
  apply Mem.stack_return_frame in H4 as SRET.
  apply Mem.stack_return_frame in RET as SRET'.
  exploit pop_stage_parallel_rule; eauto.
  apply Mem.pop_stage_nonempty in H5. inv STK. congruence.
  clear SEP. intros (m2'' & POP' & SEP).
  rewrite sep_swap in SEP.
  apply Mem.stack_pop_stage in H5 as SPOP.
  apply Mem.stack_pop_stage in POP' as SPOP'.
  apply Mem.astack_pop_stage in H5 as ASPOP.
  apply Mem.astack_pop_stage in POP' as ASPOP'.
  exploit find_function_translated; eauto.
  ...
  eapply find_function_inv; eauto.
  unfold Mem.stackeq in *. split. inv STK.
  destruct SRET. destruct SRET'. congruence.
  inv STK. apply Mem.sid_return_frame in H4. apply Mem.sid_return_frame in RET.
  apply Mem.sid_pop_stage in H5. apply Mem.sid_pop_stage in POP'.
  congruence.
  destruct ASPOP. destruct ASPOP'. congruence.
  rewrite <- (Mem.support_free _ _ _ _ _ H3). apply sinj_refl.
  etransitivity.
  eapply Mem.support_return_frame_1; eauto.
  eapply Mem.support_pop_stage_1; eauto.
  erewrite <- Mem.sid_pop_stage. 2: eauto.
  erewrite <- Mem.sid_return_frame. 2: eauto. rewrite <- SF'.
  congruence.
- (* Lbuiltin *)
  ...
  apply external_call_astack in H0. apply external_call_astack in EC.
  congruence.
{
  apply Axioms.extensionality. intro b0.
...
- (* Lreturn *)
  ...
  exploit return_frame_parallel_rule; eauto.
  eapply Mem.return_frame_active in H0. inv STK.
  congruence.
  clear SEP. intros (m2' & RET & SEP).
  apply Mem.astack_return_frame in H0 as SR. apply Mem.astack_return_frame in RET as SR'.
  exploit pop_stage_parallel_rule; eauto.
  apply Mem.pop_stage_nonempty in H1. congruence.
  clear SEP. intros (m3' & POP & SEP).
  apply Mem.astack_pop_stage in H1 as SP. apply Mem.astack_pop_stage in POP as SP'.
  econstructor; split.
  eapply plus_right. eexact D. econstructor; eauto. traceEq.
  econstructor; eauto.
  split. apply Mem.stack_return_frame in RET. apply Mem.stack_return_frame in H0.
  apply Mem.stack_pop_stage in POP. apply Mem.stack_pop_stage in H1.
  destruct RET. destruct H0. inv STK. congruence.
  apply Mem.sid_return_frame in RET. apply Mem.sid_return_frame in H0.
  apply Mem.sid_pop_stage in POP. apply Mem.sid_pop_stage in H1. inv STK. congruence.
  apply Mem.astack_return_frame in RET. apply Mem.astack_return_frame in H0.
  apply Mem.astack_pop_stage in POP. apply Mem.astack_pop_stage in H1.
  destruct POP. destruct H1. congruence.
  rewrite <- SF.
  apply sinj_refl. etransitivity.
  eapply Mem.support_return_frame_1; eauto.
  eapply Mem.support_pop_stage_1; eauto.
  rewrite sep_swap; eauto.
  erewrite <- Mem.sid_pop_stage. 2: eauto.
  erewrite <- Mem.sid_return_frame. 2: eauto.
  congruence.
...
- (* internal function *)
  ...
  clear SEP;
  intros (j' & rs' & m1' & m2' & sp' & m3' & m4' & m5' & m6' & A & B & C & D & E & F & G & I & SEP & J & K & L & M & N). auto.
  rewrite (sep_comm (globalenv_inject ge j')) in SEP.
  rewrite (sep_swap (minjection j' m''')) in SEP.
  econstructor; split.
  eapply plus_left. econstructor; eauto.
  unfold fn_stack_requirements in C. rewrite FIND in C. exact C.
  rewrite (unfold_transf_function _ _ TRANSL). unfold fn_code. unfold transl_body.
  eauto. traceEq.
  eapply match_states_intro with (j := j'); eauto with coqlib.
  eapply match_stacks_change_meminj; eauto.
  rewrite sep_swap in SEP. rewrite sep_swap. eapply stack_contents_change_meminj; eauto.
  assert (Mem.sid (Mem.support m6') = Mem.sid (Mem.support m'0)).
  inv L. rewrite <- H4. inv STK. rewrite <- H6. erewrite <- Mem.sid_record_frame. 2: eauto.
  simpl. erewrite Mem.sid_alloc. 2: eauto. erewrite <- Mem.sid_alloc_frame; eauto.
  congruence.
...
- (* external function *)
  ...
  auto. auto.
  apply external_call_astack in A. apply external_call_astack in H0. congruence.
{
  apply Axioms.extensionality. intro b0.
  destruct (Mem.sup_dec b0 (Mem.support m)).
  -
...
- intros. destruct H0.
  ...
  eapply step_type_preservation; eauto. eexact wt_prog. eapply H.
  auto.
Qed.
