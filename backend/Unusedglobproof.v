...
Proof.
  ...
  - intros.
    ...
    apply IS.MSet.Raw.bst_Ok in H4.
    apply IS.MSet.Raw.bst_Ok in H5.
    eapply IHthis1 in H;auto.
    eapply IHthis2;auto.
    eapply IS.add_2.
    auto.
Qed.
...
(* a general property *)
Lemma In_rename_builtin_args  {A :Type} : forall id a (l:list (builtin_arg A)) ,
    In id (globals_of_builtin_args l) ->
    In (alpha_rename a id)
       (globals_of_builtin_args (map (alpha_rename a) l)).
Proof. 
  intros.
  Transparent Alpha_ident Alpha_builtin_arg.
  simpl.
  induction l.
  - simpl;auto.
  - simpl in *. eapply in_app. eapply in_app in H.
    destruct H.
    + left. induction a0;simpl in *;auto.
      1-2: destruct H;subst;auto.
      eapply in_app in H. eapply in_app. destruct H.
      left. eapply IHa0_1. auto.
      right. eapply IHa0_2. auto.
      eapply in_app in H. eapply in_app. destruct H.
      left. eapply IHa0_1. auto.
      right. eapply IHa0_2. auto.
...
Lemma rename_gd_remain_ref : forall id gd a,
    ref_def gd id ->
    ref_def (alpha_rename a gd) (alpha_rename a id).
Proof.
  destruct gd;simpl.
  - destruct f;simpl.
    + destruct f. unfold ref_function.
      Transparent Alpha_def Alpha_fundef Alpha_function.
      intros.
      destruct H as [pc [i [? ?]]].
      simpl in H.
      unfold ref_function. exists pc. exists (alpha_rename a i).
      simpl.
      split.
      erewrite PTree.gmap1. rewrite H.
      simpl. auto.
      (* after rename, the renamed id is still in the ref list, detsurct the instruction to the lowest level *)
      Transparent Alpha_instruction Alpha_operation Alpha_addressing Alpha_builtin_arg.
      simpl.
      ...
      rewrite H0. auto.
      (* addressing *)
      1-4: destruct a0;simpl in *;auto;destruct H0;subst;auto.
      (* reg + ident *)
      1-2: destruct s0;simpl in *;auto;destruct H0;subst;auto.
      simpl in H0.
      (* prove H0 can derive goal, construct a lemma *)
      eapply In_rename_builtin_args.
      auto.
    + auto.
  - simpl. unfold ref_init.
    intros.
    destruct H.
    exists x.
    Transparent Alpha_vardef.
    destruct v. simpl in *.
    induction gvar_init;simpl in *;auto.
    destruct H.
    left;subst;simpl;auto.
    right. eapply IHgvar_init.
    auto.
Qed.    
...
Proof.
  ...
  split.
  set (w' := rename_wset a w IS.empty).
  exists w'.
  split.
  (* valid_used_set proof *)
  ...
  {
    (* inverse rename id to id_1, and get id_1 is in w, use lemma to prove rename id_1 = id' is in w' *)
    (* forall assumption in used_closed, we need to prove a corresponding lemma between source program and rename program. e.g. for "IS.In id w'", we prove "In_rename_wset" lemma *)
    intros.
    ...
    (* use rename_gd_remain_ref lemma*)
  ...
  { admit. }
  assert (used_public' : forall id, In id (prog_public s') -> IS.In id w').
  { admit. }
  assert (used_defined' : forall id, IS.In id w' -> In id (prog_defs_names s') \/ id = prog_main s').
  { admit. }
  eapply (Build_valid_used_set);auto.
  (* match_prog_1 proof *)
  assert (match_prog_main' : prog_main (alpha_rename a t) = prog_main s').
  { admit. }
  assert (match_prog_public' : prog_public (alpha_rename a t) = prog_public s').
  { admit. }
  assert (match_prog_def' : forall id, (prog_defmap (alpha_rename a t)) ! id = (if IS.mem id w' then (prog_defmap s') ! id else None)).
  { admit. }
  assert (match_prog_unique' : list_norepet (prog_defs_names (alpha_rename a t))).
  { admit. }
  eapply Build_match_prog_1;auto.
      
  exists a. split;auto.
  rewrite (match_prog_public _ _ _ H1).
  rewrite (match_prog_main _ _ _ H1).
  auto.
