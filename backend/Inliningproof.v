...
with match_stacks_inside (F: meminj) (m m': mem):
        nat -> list nat -> list stackframe -> list stackframe
        -> function -> context -> sup -> regset -> Prop :=
...
Lemma match_stacks_support:
  forall l stk stk' support support1,
...
Lemma match_stacks_invariant:
  forall l stk stk' support, match_stacks F m m' l stk stk' support ->
...
with match_stacks_inside_invariant:
  forall n l stk stk' f' ctx sps' rs1,
  match_stacks_inside F m m' n l stk stk' f' ctx sps' rs1 ->
  forall rs2 sp'
         (SPS: sp' = fresh_block sps')
         (RS: forall r, Plt r ctx.(dreg) -> rs2#r = rs1#r)
         (INJ: forall b1 b2 delta,
               F1 b1 = Some(b2, delta) -> sup_In b2 (sup_add sp' sps') -> F b1 = Some(b2, delta))
         (PERM1: forall b1 b2 delta ofs,
               F1 b1 = Some(b2, delta) -> sup_In b2 (sup_add sp' sps') ->
               Mem.perm m1 b1 ofs Max Nonempty -> Mem.perm m b1 ofs Max Nonempty)
         (PERM2: forall b ofs, sup_In b (sup_add sp' sps') ->
               Mem.perm m' b ofs Cur Freeable -> Mem.perm m1' b ofs Cur Freeable)
         (PERM3: forall b ofs k p, sup_In b (sup_add sp' sps') ->
               Mem.perm m1' b ofs k p -> Mem.perm m' b ofs k p),
...
(*  intros; eapply INJ; eauto. apply BELOW. right. auto.
 ...
 (* intros; eapply INJ; eauto. apply BELOW. right. auto.
  ...
  intros; eapply INJ; eauto. apply Mem.sup_add_in2. auto.
  intros; eapply PERM1; eauto. apply Mem.sup_add_in2. auto.
  intros; eapply PERM2; eauto. apply Mem.sup_add_in2. auto.
  intros; eapply PERM3; eauto. apply Mem.sup_add_in2. auto.
  (* inlined *)
  subst sp'0.
...
Lemma match_stacks_inside_alloc_left:
  forall F m m' n l stk stk' f' ctx sps' sp' rs',
...
Lemma match_stacks_free_right:
  forall F m m' l stk stk' sps sp lo hi m1',
...
with match_stacks_inside_extcall:
  forall n l stk stk' f' ctx sp' sps' rs',
  match_stacks_inside F1 m1 m1' n l stk stk' f' ctx sps' rs' ->
  sp' = fresh_block sps' ->
  Mem.sup_include (sup_add sp' sps') (Mem.support m1') ->
...
Inductive match_states: RTL.state -> RTL.state -> Prop :=
  | match_regular_states: forall stk f sp pc rs m stk' f' sps' sp' rs' m' F fenv ctx n l
        ...
        (VB: Mem.sup_include (sup_add sp' sps') (Mem.support m'))
        (PRIV: range_private F m m' sp' (ctx.(dstk) + ctx.(mstk)) f'.(fn_stacksize))
        (SSZ1: 0 <= f'.(fn_stacksize) < Ptrofs.max_unsigned)
        (SSZ2: forall ofs, Mem.perm m' sp' ofs Max Nonempty -> 0 <= ofs <= f'.(fn_stacksize))
        (SIZES: inline_sizes (S n :: l) (Mem.stack (Mem.support m)) (Mem.stack(Mem.support m'))),
      match_states (State stk f (Vptr sp Ptrofs.zero) pc rs m)
                   (State stk' f' (Vptr sp' Ptrofs.zero) (spc ctx pc) rs' m')
  | match_call_states: forall stk fd args m stk' fd' args' m' cunit F sz l
        ...
        (MINJ: Mem.inject F m m')
        (SIZES : inline_sizes (1%nat::l) (Mem.stack(Mem.support m)) (Mem.stack(Mem.support m'))),
      match_states (Callstate stk fd args m sz)
                   (Callstate stk' fd' args' m' sz)
  | match_call_regular_states: forall stk f vargs m stk' f' sps' sp' rs' m' F fenv ctx ctx' pc' pc1' rargs sz n l
        (SPS: sp' = fresh_block sps')
        (MS: match_stacks_inside F m m' n l stk stk' f' ctx sps' rs')
        (COMPAT: fenv_compat prog fenv)
        (FB: tr_funbody fenv f'.(fn_stacksize) ctx f f'.(fn_code))
...
Lemma inline_sizes_same_top:
  forall g f1 f2 s1 s2,
    ...
    Mem.size_of_all_frames f1 = Mem.size_of_all_frames f2 ->
...
Proof.
  ...
  - unfold Mem.sup_include in *. unfold Mem.sup_In in *.
    simpl in *. eapply Mem.supp_pop_stage in H0.
    rewrite <- H0. auto.
  ...
  - intros. exploit unchanged_on_contents; eauto. intros.
    ...
    unfold Mem.pop_stage in H0. destruct (Mem.stack(Mem.support m2')).
...
- (* store *)
  ...
  rewrite (Mem.support_store _ _ _ _ _ _ U).
  auto.
- (* call *)
...
+ (* not inlined *)
  ...
  econstructor; eauto.
  eapply match_stacks_cons; eauto.
  eapply match_stacks_inside_push; eauto.
...
+ (* within the original function *)
  ...
  eapply match_stacks_invariant; eauto.
    ...
    intros. eapply Mem.perm_free_3; eauto.
  erewrite Mem.support_free; eauto. eapply Mem.sup_include_trans; eauto.
  eapply agree_val_regs; eauto.
  ...
  erewrite (Mem.support_free _ _ _ _ _ H3).
  erewrite (Mem.support_free _ _ _ _ _ FREE).
  auto.
+ (* turned into a call *)
  left; econstructor; split.
  eapply plus_one. eapply exec_Icall; eauto.
...
- (* builtin *)
  ...
  apply VB.  auto.
  {
    ...
    apply Mem.stack_pop_stage in POP. destruct POP as [hd' STK2].
...
- (* internal function, not inlined *)
  ...
  rewrite Stk1 in SIZES. rewrite Stk1' in SIZES.
...
- (* internal function, inlined *)
  ...
  econstructor. auto.
  eapply match_stacks_inside_record_frame_left.
  ...
  auto. auto. auto.
  rewrite H3.
  eapply range_private_alloc_left in PRIV. 2:apply H.
  eapply range_private_invariant. apply PRIV.
  intros. split. eauto. erewrite Mem.perm_record_frame.
  eauto. eauto.
  auto.
  auto. auto. auto. auto.
...
- (* external function *)
  ...
  econstructor.
    ...
    auto. auto.
    erewrite <- external_call_mem_stackeq; eauto.
    erewrite <- (external_call_mem_stackeq _ _  _ _  _ _  _ A). eauto.
...
+ (* normal case *)
  ...
  * eapply range_private_invariant; eauto.
    ...
    intros. erewrite <- Mem.perm_pop_stage; eauto.
  * intros. apply SSZ2. erewrite Mem.perm_pop_stage; eauto.
  * apply Mem.stack_pop_stage in H.
    apply Mem.stack_pop_stage in POP.
    destruct H as [hd Hstk].
    destruct POP as [hd' Hstk'].
    assert (Mem.stack(Mem.support m') = tl (Mem.stack(Mem.support m))).
    rewrite Hstk. auto.
    assert (Mem.stack(Mem.support m'1) = tl (Mem.stack(Mem.support m'0))).
    rewrite Hstk'. auto. rewrite H. rewrite H0.
    apply inline_sizes_down. auto.
+ (* untailcall case *)
...
+ (* with a result *)
  ...
  econstructor; eauto.
  apply match_stacks_inside_set_reg; auto.
  eapply match_stacks_inside_invariant. 2:apply MS. all: eauto.
  * intros. erewrite Mem.perm_pop_stage; eauto.
  * apply agree_set_reg; auto.
  ...
  * apply Mem.stack_pop_stage in H.
    destruct H as [hd Hstk].
    assert (Mem.stack(Mem.support m') = tl (Mem.stack(Mem.support m))).
    rewrite Hstk. auto. rewrite H.
    apply inline_sizes_downstar. auto.
+ (* without a result *)
  left; econstructor; split.
  eapply plus_one. eapply exec_Inop; eauto.
  econstructor; eauto.
...
Proof.
  ...
  destruct TRANSF as (_&MAIN&_). rewrite MAIN.
  econstructor; eauto.
  instantiate (2 := Mem.flat_inj (Mem.support m0)).
  apply match_stacks_nil with (Mem.support m0).
  constructor; intros.
    unfold Mem.flat_inj. apply pred_dec_true; auto.
    unfold Mem.flat_inj in H. destruct (Mem.sup_dec b1 (Mem.support m0)); congruence.
    eapply Genv.find_symbol_not_fresh; eauto.
    eapply Genv.find_funct_ptr_not_fresh; eauto.
    eapply Genv.find_var_info_not_fresh; eauto.
    unfold Mem.sup_include. unfold sup_In. simpl. auto.
  eapply Mem.push_stage_inject.
  eapply Genv.initmem_inject; eauto.
  simpl. apply inline_sizes_up.
