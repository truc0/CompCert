...
Lemma transf_fundef_alpha: forall fd permu,
    transf_fundef (AST.alpha_rename permu fd) =
    AST.alpha_rename permu (transf_fundef fd). 
Proof.
...
Instance TransfRenumberAlpha: TransfAlpha match_prog (fun p => p.(prog_main) :: p.(prog_public)) (fun p => p.(prog_main) :: p.(prog_public)).
Proof.
  red.
  intros.
