...
Lemma eval_store:
  ...
  step fn_stack_requirements tge
       (State f (store chunk a1 a2) k sp e m)
...
Lemma push_stage_exter_pop_stage :forall ef ge vl m1 t v m2,
    external_call ef ge vl (Mem.push_stage m1) t v m2 ->
    {m3:mem| Mem.pop_stage m2 = Some m3}.
Proof.
  intros.
...
Proof.
  intros. split.
  eapply Mem.pop_stage_left_extends' in H0; eauto.
  eapply Mem.extends'_extends'_compose; eauto.
  apply H1. eapply Mem.push_stage_left_extends'; eauto.
  apply Mem.extends'_refl.
  apply external_call_mem_stackeq in H. inv H1.
  unfold Mem.stackeq in *. apply Mem.stack_pop_stage in H0.
...
Lemma classify_stmt_sound_1:
  ...
  star (Cminor.step fn_stack_requirements) ge (Cminor.State f s k sp e m) E0 (Cminor.State f Cminor.Sskip k sp e m).
Proof.
...
Lemma classify_stmt_sound_2:
  ...
  star (Cminor.step fn_stack_requirements) ge (Cminor.State f s k sp e m) E0 (Cminor.State f Cminor.Sskip k sp (PTree.set id v e) m).
Proof.
  intros until s; functional induction (classify_stmt s); intros; try discriminate.
...
Lemma if_conversion_correct:
  ...
  /\ star (Cminor.step fn_stack_requirements) ge (Cminor.State f s0 k sp e m) E0 (Cminor.State f Cminor.Sskip k sp e1 m)
  /\ env_lessdef e1 e1'.
Proof.
...
Inductive match_states: Cminor.state -> CminorSel.state -> Prop :=
  | match_state: forall cunit hf f f' s k s' k' sp e m e' m' env
      ...
      match_states
        ...
        (State f' s' k' sp e' m')
  | match_callstate: forall cunit f f' args args' k k' m m' id
        (LINK: linkorder cunit prog)
        ...
        (ME: Mem.extends m m'),
      match_states
        (Cminor.Callstate f args k m id)
        (Callstate f' args' k' m' id)
  ...
  | match_builtin_1: forall cunit hf ef args optid f sp e k m al f' e' k' m' env sz
        ...
        (ME: Mem.extends m m'),
      match_states
        (Cminor.Callstate (External ef) args (Cminor.Kcall optid f sp e k) (Mem.push_stage m) sz)
...
Lemma sel_step_correct:
  ...
  \/ (exists S3 T2, star (Cminor.step fn_stack_requirements) ge S2 E0 S3 /\ step fn_stack_requirements tge T1 t T2 /\ match_states S3 T2).
Proof.
...
+ (* direct *)
  ...
  destruct H as (bb & oo & EQ & EQ'). subst. inv V.
  left; econstructor; split.
  apply plus_one; econstructor; eauto.
...
-  (* external call turned into a Sbuiltin *)
  ...
  eapply push_stage_exter_pop_stage; eauto. destruct X as [m'' POP_STAGE].
...
Proof.
  ...
  destruct TRANSF as (_ & MAIN & _); rewrite MAIN. econstructor; eauto.
...
- intros. exploit sel_initial_states; eauto. intros (T & P & Q).
  exists T; split; auto; split; auto. eapply wt_initial_state. eexact wt_prog. apply H.
- intros. destruct H. eapply sel_final_states; eauto.
- intros S1 t S2 A T1 [B C].
  assert (wt_state S2). { eapply subject_reduction; eauto using wt_prog. apply A. }
  unfold MS.
