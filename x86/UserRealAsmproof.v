...
Require Import UserAsmFacts.
Require Import RealAsm UserRealAsm UserRealAsmgen.
...
Require Import Linking.
  ...
  Definition pc_at_1 (s: state): option ((function * instruction) + external_function) :=
    let '(State rs m) := s in
    match rs PC with
    | Vptr b o =>
  ...
  Inductive is_builtin_1: instruction -> Prop :=
  | is_builtin_intro ef args res: is_builtin_1 (Pbuiltin ef args res).

  Lemma is_builtin_1_dec i: is_builtin_1 i \/ ~ is_builtin_1 i.
  Proof.
  ...
  (* Context `{tprog: UserAsm.program}. *)
  Variable tprog: UserAsm.program.
  Hypothesis TRANSF2: prog_no_jmp_rel tprog.
  Let tge := Genv.globalenv tprog.
  Import UserAsm.

  Lemma find_jmp_rel_absurd_2: forall b f ofs i,
      Genv.find_funct_ptr tge b = Some (Internal f) ->
  ...
  Proof.
    ...
    intros (id & IN).
    red in TRANSF2.
    rewrite Forall_forall in TRANSF2.
    exploit TRANSF2; eauto. simpl.
    exploit find_instr_in; eauto.
    intros INi NJ.
    red in NJ.
  ...
  Definition pc_at_2 (s: state): option ((function * instruction) + external_function) :=
    let '(State rs m) := s in
    match rs PC with
    | Vptr b o =>
  ...
  Inductive is_builtin_2: instruction -> Prop :=
  | is_builtin_2_intro ef args res: is_builtin_2 (Pbuiltin ef args res).

  Lemma is_builtin_2_dec i: is_builtin_2 i \/ ~ is_builtin_2 i.
  Proof.
    destruct i; first [right; now inversion 1|now econstructor].
  Qed.
  ...
  Definition match_prog (p: Asm.program) (tp: UserAsm.program) :=
    match_program (fun _ f tf => transf_fundef f = tf) eq p tp.
  ...
  Lemma match_prog_inv: forall t tp,
    match_prog t tp -> transf_program t = tp.
  Admitted.
    (* intros t tp MT. red in MT. red in MT. red in MT. *)
    (* destruct MT as (MT & EQ1 & EQ2). *)
    (* set (P := *)
    (*        (match_ident_globdef (fun (_ : program Asm.fundef unit) (f : Asm.fundef) (tf : UserAsm.fundef) => transf_fundef f = tf) *)
    (*                                    (@eq unit) t)) in *. *)
  ...
  Hypothesis prog_no_rsp: RealAsm.asm_prog_no_rsp ge.
  Hypothesis TRANSF2: prog_no_jmp_rel tprog.
  Let tge := Genv.globalenv tprog.

  (* Import Asm RealAsm. *)
  Coercion Asm.IR: Asm.ireg >-> Asm.preg.
  Coercion Asm.FR: Asm.freg >-> Asm.preg.
  Coercion Asm.CR: Asm.crbit >-> Asm.preg.

  Inductive seq: Asm.state -> UserAsm.state -> Prop :=
  | seq_intro rs1 rs2 m (REQ: forall r, r <> Asm.RA -> rs1 r = rs2 (transf_preg r)): seq (Asm.State rs1 m) (UserAsm.State rs2 m).

  Inductive match_states: Asm.state -> UserAsm.state -> Prop :=
  ...
  | match_states_normal s1 s2
                        (SEQ: seq s1 s2)
                        (PC1: match pc_at_2 tprog s2 with
                              | Some (inl (f,i)) => ~ UserRealAsm.intermediate_instruction i
                              | Some (inr ef) => False
                              | None => True
                              end
                        ):
  ...
  | match_states_stuck s1 s2
                       (PCnone: pc_at_1 prog s1 = None)
                       (PCeq: RealAsm.rs_state s1 Asm.PC = UserRealAsm.rs_state s2 UserAsm.PC)
                       (RAXeq: RealAsm.rs_state s1 Asm.RAX = UserRealAsm.rs_state s2 UserAsm.RAX)
    : match_states s1 s2.
  ...
  Lemma initial_states_match rs2:
    forall s1 s2,
      RealAsm.initial_state prog s1 ->
      initial_state tprog rs2 s2 ->
      exists s1', RealAsm.initial_state prog s1' /\ match_states s1' s2.
  Admitted.
  (*
  Proof.
  ...
  *)
  ...
  Lemma match_states_PC:
    forall s1 s2,
      match_states s1 s2 ->
      RealAsm.rs_state s1 Asm.PC = rs_state s2 UserAsm.PC.
  Proof.
  ...
  Lemma step_internal:
    forall s t s'
      (STEP: RealAsm.step ge s t s')
      f i
      (PCAT: pc_at_1 prog s = Some (inl (f,i)))
      (NB: ~ is_builtin_1 i),
      RealAsm.exec_instr ge f i (RealAsm.rs_state s) (RealAsm.m_state s) = Asm.Next (RealAsm.rs_state s') (RealAsm.m_state s') /\ t = E0.
  Admitted.
  (*
    unfold pc_at_1; intros s t s'  STEP; inv STEP.
  ...
  *)
  ...
  Lemma step_external_store:
    forall s t s'
      (STEP: RealAsm.step ge s t s')
      ef
      (PCAT: pc_at_1 prog s = Some (inr ef)),
    exists m2,
      Mem.storev Mptr (RealAsm.m_state s) (Val.offset_ptr (RealAsm.rs_state s Asm.RSP) (Ptrofs.neg (Ptrofs.repr (size_chunk Mptr))))
                 (RealAsm.rs_state s Asm.RA) = Some m2.
  Admitted.
  (*
    unfold pc_at_1; intros s t s'  STEP; inv STEP; rewrite H, H0; try now destr.
    inversion 1; simpl; eauto.
  *)

  Lemma internal_step:
    forall rs rs' m m' f i
      (PCAT: pc_at_1 prog (Asm.State rs m) = Some (inl (f,i)))
      (EI: RealAsm.exec_instr ge f i rs m = Asm.Next rs' m'),
      RealAsm.step ge (Asm.State rs m) E0 (Asm.State rs' m').
  Proof.
    intros.
  ...
  Lemma real_asm_progress:
    forall s1 s2,
      match_states s1 s2 ->
      safe (RealAsm.semantics prog) s1 ->
      real_asm_inv s2 ->
  ...
  Proof.
    ...
    simpl.
    inv SEQ.
    assert (RealAsm.exec_instr ge f i rs2 m2 = Next rs2' m2').
    {
      rewrite <- EI.
      destruct i; auto.
  ...
  Lemma real_asm_step:
    forall s2 t s2',
      ...
      forall s1 : state,
        ...
        real_asm_inv s2 ->
        safe (RealAsm.semantics prog) s1 ->
        exists s1', RealAsm.step (Genv.globalenv prog) s1 t s1' /\ match_states s1' s2'.
  Proof.
    ...
    - (*Pallocframe*)
      ...
      eexists; split. eapply RealAsm.exec_step_internal. rewrite REQ by congruence; eauto.
      eauto. eauto.
    ...
    - (* external call *)
      ...
      eexists; split. eapply RealAsm.exec_step_external. rewrite REQ by congruence; eauto.
      eauto.
      rewrite <- MEQ.
    ...
    - (*Pret*)
      ...
      eexists; split. eapply RealAsm.exec_step_internal. rewrite REQ by congruence; eauto. eauto. eauto.
    ...
    - (*common case*)
      ...
      + inv SEQ. eexists; split. eapply RealAsm.exec_step_builtin; eauto.
        rewrite REQ by congruence; eauto.
  ...
  Theorem real_asm_correct:
    backward_simulation (RealAsm.semantics prog) (UserRealAsm.semantics tprog).
  Proof.
