...
Definition find_ext_funct (ge: t) (v:val) : option external_function :=
  match v with
  | Vptr (Global bb) ofs => PTree.get bb ge.(genv_ext_funs )
  | _ => None
  end.
...
Definition add_external_global (extfuns: PTree.t external_function) 
  ...
  let gsymbs := 
      if is_symbentry_internal e then
        ...
        PTree.set id (Global id(*Genv.genv_next ge*), Ptrofs.zero)
                  (Genv.genv_symb ge)
  in
  let gextfuns :=
      ...
      | symb_func =>
        if is_symbentry_internal e then
          ...
          | Some ef =>
            PTree.set (id(*Genv.genv_next ge*)) ef (Genv.genv_ext_funs ge)
          end
  ...
  let bnext := 
      ...
      else Pos.succ (Genv.genv_next ge)
  in
  *)
  Genv.mkgenv
    gsymbs
...
Definition acc_extfuns (idg: ident * gdef) extfuns :=
  let '(id, gdef) := idg in
  match gdef with
  | Gfun (External ef) => PTree.set id ef extfuns
  | _ => extfuns
  end.
