...
Inductive reduction: Type :=
  ...
  | Callred (rule: string) (fd: fundef) (args: list val) (tyres: type) (m': mem)(id:ident)
  | Stuckred.
  ...
  | RV, Ecall r1 rargs ty =>
      ...
      | Some(vf, tyf), Some vtl =>
          ...
          | fun_case_f tyargs tyres cconv =>
              ...
              topred (Callred "red_call" fd vargs ty m id)
          | _ => stuck
          end
      | _, _ =>
...
Inductive imm_safe_t: kind -> expr -> mem -> Prop :=
  ...
  | imm_safe_t_rred: forall to C r m t r' m' w',
      ...
      context RV to C ->
      imm_safe_t to (C r) m
  | imm_safe_t_callred: forall to C r m fd args ty id,
...
Definition reduction_ok (k: kind) (a: expr) (m: mem) (rd: reduction) : Prop :=
  ...
  | RV, Callred _ fd args tyres m' id => callred ge a m fd args tyres id /\ m' = m
...
(* call *)
  ...
  destruct (invert_pointer vf) as [ifi|] eqn:?...
  ...
  eexists. eexists. split. eauto. eapply Genv.invert_find_symbol. eauto.
  ...
  apply not_invert_ok; simpl; intros; myinv. specialize (H ALLVAL). myinv. congruence.
  apply not_invert_ok; simpl; intros; myinv. specialize (H ALLVAL). myinv.
  destruct vf eqn: Hvf; simpl in *; try congruence.
  destruct H as (bb&oo&A&B). inv A.
  apply Genv.find_invert_symbol in B. congruence.
  apply not_invert_ok; simpl; intros; myinv. specialize (H ALLVAL). myinv. congruence.
  (* depth *)
...
Lemma callred_topred:
  forall a fd args ty m id,
  callred ge a m fd args ty id->
  exists rule, step_expr RV a m = topred (Callred rule fd args ty m id).
Proof.
  induction 1; simpl.
  rewrite H3. exploit sem_cast_arguments_complete; eauto. intros [vtl [A B]].
  assert (C:invert_pointer vf = Some id).
   destruct H as (bb&oo&AA&BB). rewrite AA. simpl. eapply Genv.find_invert_symbol. eauto.
  rewrite A; rewrite C; rewrite H0; rewrite B; rewrite H2; rewrite dec_eq_true. econstructor; eauto.
Qed.
...
Definition expr_final_state (f: function) (k: cont) (e: env) (C_rd: (expr -> expr) * reduction) :=
  ...
  | Callred rule fd vargs ty m id => TR rule E0 (Callstate fd vargs (Kcall f e (fst C_rd) ty k) m (fn_stack_requirements id))
...
Definition do_step (w: world) (s: state) : list transition :=
  ...
  | State f (Sreturn None) k e m =>
      do m' <- Mem.free_list m (blocks_of_env ge e);
      match (Mem.pop_stage m') with
        |Some m'' =>  ret "step_return_0" (Returnstate Vundef (call_cont k) m'')
        |None => nil
      end
  ...
  | State f Sskip ((Kstop | Kcall _ _ _ _ _) as k) e m =>
      do m' <- Mem.free_list m (blocks_of_env ge e);
      match (Mem.pop_stage m') with
        |Some m'' =>      ret "step_skip_call" (Returnstate Vundef k m'')
        |None => nil
      end
  ...
  | Callstate (External ef _ _ _) vargs k m sz =>
      match do_external ef w vargs m with
      | None => nil
...
Theorem do_step_sound:
  ...
  Csem.step fn_stack_requirements ge S t S' \/ (t = E0 /\ S' = Stuckstate /\ can_crash_world  w S).
...
(* callstate *)
  ...
  apply do_alloc_variables_sound. rewrite Heqp. simpl. apply Heqo.
...
(* callred *)
  ...
  change (TR rule E0 (Callstate fd vargs (Kcall f e C ty k) m (fn_stack_requirements
id)) )with (expr_final_state f k e (C, Callred rule fd vargs ty m id)).
  apply in_map.
  generalize (step_expr_context e w _ _ _ H1 a m). unfold reducts_incl.
  intro. replace C with (fun x => C x). apply H2.
  rewrite STEP; unfold topred; auto with coqlib.
...
Definition do_initial_state (p: program): option (genv * state) :=
  ...
  Some (ge, Callstate f nil Kstop m0 (fn_stack_requirements p.(prog_main))).
